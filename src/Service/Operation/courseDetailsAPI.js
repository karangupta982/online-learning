...
export const createRating = async (data, token) => {
  ...
  let success = false
  try {
    response = await apiConnector("POST", CREATE_RATING_API, data, {
      Authorization: `Bearer ${token}`,
    })
...
export const createReview = async (data, token) => {
  if(data.rating === 0) return toast.error("Rating can not be 0, Add review again")
  if(data.review.length === 0)return toast.error("Please add some review")
  const toastId = toast.loading("Loading...")
  let success = false
  try {
